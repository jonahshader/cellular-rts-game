...
import jonahshader.game.SoundManager;

import java.util.ArrayList;

import static jonahshader.game.Utils.interpolate;
...
public class Menu implements InputProcessor {
    ...
    public void addMenuItem(Action action, String label) {
        MenuItem newItem = new MenuItem(action, label, menuItems.size());
...
//            index = 0;
        if (keycode == Input.Keys.UP) {
            ...
            if (index > -1) {
                menuItems.get(index).action.executeAction();
    ...
    public class MenuItem {
        ...
        public MenuItem(Action action, String label, int id) {
            this.action = action;
            this.label = label;
            this.id = id;
            x = firstX;
